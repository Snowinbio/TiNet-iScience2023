...
class ISAIDSegmmDataset(PatchBasedDataset):
    ...
    def generate_path_pair(self):
        ...
        image_path_list = glob.glob(os.path.join(self.image_dir, '*.jpg'))

        mask_path_list = [os.path.join(self.mask_dir, os.path.basename(imfp)) for imfp in image_path_list]
...
class ImageFolderDataset(Dataset):
    def __init__(self, image_dir, mask_dir=None):
        self.fp_list = glob.glob(os.path.join(image_dir, '*.jpg'))
    ...
    def __getitem__(self, idx):
        ...
        if self.mask_dir is not None:
            mask_fp = os.path.join(self.mask_dir, os.path.basename(self.fp_list[idx]))
